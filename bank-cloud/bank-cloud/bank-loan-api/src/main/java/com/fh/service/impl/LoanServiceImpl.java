...
import com.fh.mapper.BankLoanMapper;
import com.fh.mapper.BankLoanPaylogMapper;
import com.fh.mapper.BankLoanPaymentMapper;
import com.fh.mapper.BankLoanTypeMapper;
...
public class LoanServiceImpl implements LoanService {
    ...
    @Autowired
    private BankAccountAPI bankAccountMapper;
    @Autowired
    private BankCustomerAPI bankCustomerMapper;
    @Autowired
     ...
     */
    @Override
    public ResponseServer dealApplyment(BankLoanApplyInfo bankLoanApplyInfo) {

        machineId = 1L;

        //判断用法合理性
        String account = bankLoanApplyInfo.getAccount();
        ResponseServer isLegalInfo = isLegalInfo(account, bankLoanApplyInfo.getName(), bankLoanApplyInfo.getIdCard(),
...
//        }
        ...
        switch (bankLoanApplyInfo.getLoanType()) {
            ...
            case ("3"):
                ...
                break;
            default:
                //return BankResult.build(400, "贷款类型不存在！");
                return ResponseServer.error(ResponseEnum.DATE_TABLE_ERROR);
        }
        try {
...
//            jedisClient.set(REDIS_SESSION_KEY+ LOAN + string, JsonUtils.objectToJson(bankLoan));
    ...
    public ResponseServer payForLoan(BankLoanPaymentInfo bankLoanPaymentInfo) {
        machineId = 2L;
        // 查询信息是否异常
        ResponseServer isLegalInfo = isLegalInfo(bankLoanPaymentInfo.getAccount(),
        ...
        try {
            ...
            BankLoan bankLoan = JsonUtils.jsonToPojo((String) redisTemplate.opsForValue().get(REDIS_SESSION_KEY + LOAN + transId), BankLoan.class);
            if (bankLoan == null) {
            ...
            List<BankLoanPayment> bankLoanPaymentList = JsonUtils.jsonToList(
                    (String) redisTemplate.opsForValue().get(REDIS_SESSION_KEY + LOAN_PAYMENT + transId), BankLoanPayment.class);
            ...
            try {
                ...
                redisTemplate.opsForValue().set(REDIS_SESSION_KEY + LOAN + string, JsonUtils.objectToJson(bankLoan));
            } catch (Exception e) {
                //return BankResult.build(400, "数据表更新异常");
                return ResponseServer.error(ResponseEnum.DATE_TABLE_ERROR);
            }
            ...
            bankLoanPaymentList = bankLoanPaymentMapper.selectByExample(bankLoanPaymentExample);
            //@REDIS
            redisTemplate.opsForValue().set(REDIS_SESSION_KEY + LOAN_PAYMENT + transId, JsonUtils.objectToJson(bankLoanPaymentList));
        } catch (Exception e) {
     ...
     */
    @Override
    public ResponseServer sentAllRecords() {
        try {
            ...
            return ResponseServer.success(bankLoanList);
        } catch (Exception e) {
     ...
     */
    @Override
    public ResponseServer sentOneRecord(String transId) {
        try {
            //@REDIS
            String str = (String) redisTemplate.opsForValue().get(REDIS_SESSION_KEY + LOAN + transId);
            ...
            if (bankLoan == null) {
                ...
                return ResponseServer.error(ResponseEnum.LOAN_NUMBER_NOT_EXIT);
            }
            redisTemplate.opsForValue().set(REDIS_SESSION_KEY + LOAN + transId, JsonUtils.objectToJson(bankLoan));
            // return BankResult.ok(bankLoan);
            return ResponseServer.success(bankLoan);
        } catch (Exception e) {
            // return BankResult.build(400,"数据表查询异常");
            return ResponseServer.error(ResponseEnum.DATE_TABLE_ERROR);
        }
    ...
    public ResponseServer getInterestRate() {
        try {
            ...
            String str = (String) redisTemplate.opsForValue().get(REDIS_SESSION_KEY + LOAN_TYPE);
            //@REDIS
            if (str != null) {
            ...
            if (bankLoanTypeList == null) {
                ...
                redisTemplate.opsForValue().set(REDIS_SESSION_KEY + LOAN_TYPE, JsonUtils.objectToJson(bankLoanTypeList));
            }
            //return BankResult.ok(bankLoanTypeList);
            return ResponseServer.success(bankLoanTypeList);
        } catch (Exception e) {
            // return BankResult.build(400,"数据表查询异常");
            return ResponseServer.error(ResponseEnum.DATE_TABLE_ERROR);
        }
    ...
    @Override
    public ResponseServer getInterestOneRate(int value) {
        String text = null;
        switch (value) {
            case 1:
                text = "住房贷款";
            ...
            case 3:
                ...
                break;
            default:
                //return BankResult.build(400,"请求错误");
                return ResponseServer.error(ResponseEnum.REQUEST_ERROR);
        }
        try {
            //@REDIS
            String str = (String) redisTemplate.opsForValue().get(REDIS_SESSION_KEY + LOAN_TYPE + value);
            ...
            if (bankLoanType == null) {
                ...
                redisTemplate.opsForValue().set(REDIS_SESSION_KEY + LOAN_TYPE + value, JsonUtils.objectToJson(bankLoanType));
            }
            // return BankResult.ok(bankLoanType);
            return ResponseServer.success(bankLoanType);
        } catch (Exception e) {
            // return BankResult.build(400,"数据表查询异常");
            return ResponseServer.error(ResponseEnum.DATE_TABLE_ERROR);
        }
    ...
    @Override
    public ResponseServer getPaylog(String logId) {
        try {
            ...
            return ResponseServer.success(bankLoanPaylogs);
        } catch (Exception e) {
            //return BankResult.build(400,"数据表查询异常");
            return ResponseServer.error(ResponseEnum.DATE_TABLE_ERROR);
        }
    ...
    @Override
    public ResponseServer getPaymentOneInfo(String transId) {
        try {
            String str =(String) redisTemplate.opsForValue().get(REDIS_SESSION_KEY + LOAN_PAYMENT + transId);
            ...
            return ResponseServer.success(bankLoanPaymentList);
     ...
     */
    public ResponseServer isLegalInfo(String account, String custName, String IdCard, String password,
        ...
        try {
            ...
            if (flag) {
                if (Double.valueOf(amount) > Double.valueOf(bankAccount.getBalances())) {
                    ...
                    return ResponseServer.error(ResponseEnum.REMIT_INSUFFICIENT_REPAYMENT_BALANCE);
                }
            }
